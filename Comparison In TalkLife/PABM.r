...
file=read.table(file="data1.txt",header=T)
n=unique(c(file$node1,file$node2))
n=n[order(n)]
indexlist=data.frame(node=n,index=c(1:length(n)))
file=merge(file,indexlist,by.x="node1",by.y="node")
file=merge(file,indexlist,by.x="node2",by.y="node")
colnames(file)=c("node2","node1","cs1","cs2","Q_ID","index_s","index_r")
K=2
A=matrix(0,length(n),length(n))
for (i in c(1:nrow(file))){
	tmp=file[i,]
	A[tmp$index_s,tmp$index_r]=1
	A[tmp$index_r,tmp$index_s]=1
}
