...
for (K in c(2,4,6,8,10)){
	#test=read.table(file=paste0("58_58.cs.",K,".txt"),header=F)
	#truth=read.table(file="58_58.txt",header=T)
	possibleError=tryCatch({
		test=read.table(file=paste0("58_58.global.prior.cs.",K,".txt"),header=F,sep="\n")
		truth=read.table(file="58_58.txt",header=T)
	}, error=function(e) e)
